...
void Network::begin()
{
    esp32_log_level_set(tag, ESP_LOG_DEBUG);
    ESP32_LOGD(tag, "network 初始化开始");
    if (!connectStoredWiFi())
    {
        ESP32_LOGD(tag, "network 未获取到Wifi链接信息");
        startConfigPortal();
    }
    ESP32_LOGD(tag, "network 初始化结束");
}

bool Network::confirmIsConnected() {
    if (!isConnected()) {
        ESP32_LOGI(tag, "网络未连接，进行重连");
        connectStoredWiFi();
    }
    ESP32_LOGI(tag, "重连中的IP地址为: %s", WiFi.localIP().toString());
    return isConnected();
}
...
bool Network::connectStoredWiFi()
{
    ...
    ESP32_LOGD(tag, "获取配置中的wifi: %s", wifiConfig.c_str());
    ...
    ESP32_LOGD(tag, "获取配置中的wifi解析出的内容 ssid: %s password: %s", ssid, password);
    ...
    ESP32_LOGD(tag, "等待链接wifi结束");
    unsigned long startTime = millis();
    while (WiFi.status() != WL_CONNECTED && millis() - startTime < WIFI_TIMEOUT)
    {
        delay(500);
    }
    ESP32_LOGD(tag, "等待链接wifi结束，链接状态为:%d", WiFi.status());
    return isConnected();
}
...
void Network::handleSave()
{
    ...
    ESP32_LOGD(tag, "配置好的wifi ssid:%s password:%s", ssid.c_str(), password.c_str());
    ...
    if (ssid.length() > 0)
    {
        ...
        ESP32_LOGD(tag, "配置好的wifi 存储的json:%s", json.c_str());
